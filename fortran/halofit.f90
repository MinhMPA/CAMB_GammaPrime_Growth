...
101                 continue
    ...
    subroutine fill_plintab(iz,cosm,CAMB_PK,gamma_prime)
    ...
    if (gamma_prime) then
        cosm%grow2_z = (growint(z,cosm,.TRUE.))**2
        !Set grow2_z=D^2(z,gamma), D comes from the numerical integral with gamma approx.
    else
        cosm%grow2_z = g**2
        !Set grow2_z=D^2(z), D comes from the ODE solution
    end if
    cosm%this_z = z
