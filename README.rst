...
This is a fork of the original CAMB code, whose original README.srt reads as below this box. We modify the original code such that the nonlinear :math:`P(k,z)` accounts for modified linear growth rate of large-scale structure of the form :math:`f(z)=\Omega_m(z)^{\gamma(z)}`, where :math:`\gamma(z)=\gamma_0+\gamma_1 z^2`.
