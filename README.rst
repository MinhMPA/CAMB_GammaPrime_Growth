...
This is a fork of the original CAMB code, whose original README.srt reads as below this box. We modify the original code such that the nonlinear $P(k,z)$ accounts for modified linear growth rate of large-scale structure, of the form $f(z)=\Omega_m(z)^{\gamma(z)}$, where $\gamma(z)=\gamma_0+\gamma_1 z^2$.
